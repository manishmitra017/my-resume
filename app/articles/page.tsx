        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl border border-text-primary/5 dark:border-white/5 p-5 text-center shadow-card">
              ...
              <div className="text-text-muted dark:text-gray-400 text-sm">Articles</div>
            </div>
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl border border-text-primary/5 dark:border-white/5 p-5 text-center shadow-card">
              <div className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-teal-400 to-navy-500 bg-clip-text text-transparent">
                {articlesData.reduce((acc, article) => acc + article.readTime, 0)}
              </div>
              <div className="text-text-muted dark:text-gray-400 text-sm">Min Read Time</div>
            </div>
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl border border-text-primary/5 dark:border-white/5 p-5 text-center shadow-card">
              <div className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-teal-400 to-navy-500 bg-clip-text text-transparent">
                {allTags.length}
              </div>
              <div className="text-text-muted dark:text-gray-400 text-sm">Topics</div>
            </div>
          </motion.div>
          ...
          >
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${selectedTag === null
                  ...
                  : "bg-white/80 dark:bg-gray-800/80 text-text-secondary dark:text-gray-300 hover:bg-cream-200/50 dark:hover:bg-gray-700/50 border border-text-primary/5 dark:border-white/5"
                }`}
            >
              All
            </button>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredArticles.map((article, index) => (
              ...
              >
                ...
                >
                  <div className="h-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl border border-text-primary/5 dark:border-white/5 hover:border-teal-400/30 hover:shadow-card-hover transition-all duration-300 overflow-hidden relative shadow-card">
                    {/* Featured Badge */}
                    {article.featured && (
                      <div className="absolute top-4 right-4 z-10">
                        <span className="bg-coral-400 text-white text-xs font-medium px-2.5 py-1 rounded-full">
                          Featured
                        </span>
                      </div>
                    )}
                    ...
                    {/* Gradient Header */}
                    <div className="h-32 bg-gradient-to-br from-teal-400/20 via-coral-400/10 to-golden-400/20 dark:from-teal-400/30 dark:via-coral-400/20 dark:to-golden-400/30 flex items-center justify-center">
                    ...
                    <div className="p-5">
                      ...
                      {/* Title */}
                      <h3 className="text-lg font-bold text-text-primary dark:text-white mb-2 group-hover:text-teal-500 transition-colors line-clamp-2">
                        {article.title}
                      </h3>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {article.tags.slice(0, 2).map((tag) => (
                          <span
                            key={tag}
                            className="text-xs bg-cream-200/50 dark:bg-gray-700/50 text-text-secondary dark:text-gray-300 px-2 py-0.5 rounded"
                          >
                            {tag}
                          </span>
                        ))}
                      ...
                      {/* Footer */}
                      <div className="flex items-center justify-between pt-4 border-t border-text-primary/5 dark:border-white/5">
